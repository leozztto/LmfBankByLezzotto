...
describe("TransferForm", () => {
  beforeEach(() => {
    ...
    // ADMIN (ADR 0010): estes testes exercitam o seletor em dropdown, que só o
    // admin vê — o caminho do usuário comum (digitar o número da conta) tem
    // sua própria describe mais abaixo.
...
describe("TransferForm · usuário comum (ADR 0010)", () => {
  beforeEach(() => {
    useUiStore.setState({ selectedAccountId: 1 });
    useAuthStore.setState({
      session: { authenticated: true, username: "demo", role: "USER" },
    });
    server.use(
      http.get("/api/accounts", () => HttpResponse.json([acc(1)])),
      http.get("/api/accounts/number/:accountNumber", ({ params }) =>
        params.accountNumber === "00000002-2"
          ? HttpResponse.json({
              accountId: 2,
              accountNumber: "00000002-2",
              fullName: "Conta 2",
            })
          : HttpResponse.json(
              { status: 404, code: "ACCOUNT_NOT_FOUND", message: "not found" },
              { status: 404 },
            ),
      ),
    );
  ...
  it("resolve o destino digitando o número da conta, sem dropdown", async () => {
    const user = userEvent.setup();
    let posted: Record<string, unknown> | undefined;
    server.use(
      http.post("/api/transfers", async ({ request }) => {
    ...
    expect(
      screen.queryByRole("combobox", { name: "Conta de destino" }),
    ).not.toBeInTheDocument();

    const destino = screen.getByLabelText("Conta de destino");

    // número incompleto: não dispara o lookup (evita varrer contas tecla a tecla)
    await user.type(destino, "00000002");
    await new Promise((r) => setTimeout(r, 50));
    expect(screen.queryByText(/Destino:/)).not.toBeInTheDocument();
    expect(screen.queryByText("Conta não encontrada")).not.toBeInTheDocument();

    await user.type(destino, "-2");
    expect(await screen.findByText("Destino: Conta 2")).toBeInTheDocument();

    await user.type(screen.getByLabelText("Valor"), "5000");
    await user.click(screen.getByRole("button", { name: "Transferir" }));
    const dialog = await screen.findByRole("dialog");
    await user.click(
      within(dialog).getByRole("button", { name: "Transferir" }),
    );

    await waitFor(() =>
      expect(posted).toMatchObject({ fromAccountId: 1, toAccountId: 2 }),
    );
  });
});
